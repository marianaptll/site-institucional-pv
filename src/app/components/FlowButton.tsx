 ...
 */
export function FlowButton({ text = 'Faça sua simulação', onClick, className = '', size = 'md' }: FlowButtonProps) {
  const isLg = size === 'lg';
  return (
    <button
      onClick={onClick}
    ...
    >
      ...
      <span className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-[#1E3A8A] rounded-[50%] opacity-0 group-hover:opacity-100 transition-all duration-[800ms] ease-[cubic-bezier(0.19,1,0.22,1)] ${isLg ? 'w-5 h-5 group-hover:w-[300px] group-hover:h-[300px]' : 'w-4 h-4 group-hover:w-[220px] group-hover:h-[220px]'}`} />
