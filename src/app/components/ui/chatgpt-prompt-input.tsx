...
export const PromptBox = React.forwardRef<
  HTMLTextAreaElement,
  React.TextareaHTMLAttributes<HTMLTextAreaElement> & { onSend?: (message: string) => void }
>(({ className, onSend, ...props }, ref) => {
  ...
  return (
    <div className={cn("flex flex-col rounded-[28px] p-2 shadow-sm transition-colors bg-white border cursor-text", className)}>
        ...
        <Dialog open={isImageDialogOpen} onOpenChange={setIsImageDialogOpen}>
          <div className="relative mb-1 w-fit rounded-[1rem] px-1 pt-1">
            ...
            <button
              ...
              className="absolute right-2 top-2 z-10 flex h-4 w-4 items-center justify-center rounded-full bg-white/50 transition-colors hover:bg-gray-100"
          ...
          <DialogContent>
            <img src={imagePreview} alt="Preview completo" className="w-full max-h-[95vh] object-contain rounded-[24px]" />
          </DialogContent>
        </Dialog>
      )}
      ...
      <textarea
        ...
        className="w-full resize-none border-0 bg-transparent p-3 text-gray-900 placeholder:text-gray-400 focus:ring-0 focus-visible:outline-none min-h-12 text-sm"
        {...props}
      />
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <button
                    ...
                    onClick={handleSend}
                    disabled={!hasValue}
                    className="flex h-8 w-8 items-center justify-center rounded-full text-sm font-medium transition-colors focus-visible:outline-none disabled:pointer-events-none bg-black text-white hover:bg-black/80 disabled:bg-black/30"
                  >
                    <SendIcon className="h-5 w-5" />
                    <span className="sr-only">Enviar</span>
                  </button>
                </TooltipTrigger>
                <TooltipContent side="top" showArrow><p>Enviar</p></TooltipContent>
              </Tooltip>
            </div>
          </div>
        </TooltipProvider>
